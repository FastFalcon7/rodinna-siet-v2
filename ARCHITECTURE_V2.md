...
| Video prílohy na iPhone | transkód (H.264/AAC) + podpísaný token v URL malo zaručiť prehrateľnosť | príčina nájdená (júl 2026): middleware reťaz + vnorené routery prebalia Response a Bun 1.2 pri Blob výseku/streame pošle na range request celý súbor s plným `Content-Length` → iOS AVFoundation probe `bytes=0-1` odpoveď odmietol (Chrome toleroval); opravené — výsek sa číta do pamäte (`arrayBuffer`, strop 8 MB) | detailný rozbor v `docs/DEPLOY_RUNBOOK.md` §9 |
